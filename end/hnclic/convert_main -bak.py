...
def load_from_url2_forJson(data_from=None,config_data=None,user_input_form_data=None,upload_path=None,userid=None):
    ...
    # http://gxapp.hn.clic/gxapp/login.jsp username: ccccc inputPassword: ccccc
    form_inputs=''
...
async def load_from_url2(data_from=None,config_data=None,user_input_form_data=None,upload_path=None,userid=None):
    ...
    '''
    for i in range(1,30):
        html_text,html_status=await _load_html_from_url(data_from,config_data,user_input_form_data,userid)
    ...
    try:
        try:#新版报表可直接返回json，先按json处理，如果不能处理，就按老版本的方式处理
            ...
            form_inputs={x['name']:"默认值" for x in (*soup.select('form input'),*soup.select('form select')) }
        ...
        for p in data_from['ds']:
            if(data_from['type']=='ZYT'):
                ...
                if isinstance(soup['data'],list):
                    data=[x for x in soup['data'] if x!=None and x['name']==pattern][0]
                else:
...
async def _load_html_from_url(data_from=None,config_data=None,user_input_form_data=None,userid=None):
    ...
    #session = requests.session()
        ...
        if data_from.get('grant_url') is not None and data_from.get('grant_url').strip()!="" :#授权url，获取cookies ，让取数url带过去
            ...
            async with session.get(data_from['grant_url']) if (real_form_data==None or len(real_form_data) ==0) else session.post(data_from['grant_url'],real_form_data) \
                 as text_html:
        ...
        async with session.get(url,cookies=cookies,headers=headers) if (real_form_data==None or len(real_form_data) ==0) else session.post(url,data=real_form_data,cookies=cookies,headers=headers) \
            as text_html:
            end_time = time.time()
            print(f'{url}取数用时： {end_time-start_time}')
            text=await text_html.text()
            if text.startswith("\ufeff"):
                text=text[1:]
            return text,text_html.status

def _guess_ds(soup):
    '''
    自动猜数据定义
    '''
    one_table=[x for x in soup.select("table") if x.has_attr('id') and x.attrs['id'].endswith('thetable')][0]
    if one_table.has_attr('data-options'):
        if soup.text.find("/*-end-*/")>0:
            t_p={"t": "json","pattern": "#"+one_table.attrs['id'],"start": one_table.attrs['id'][0:-len("thetable")] + "_data={\"rows\":",
                "end": "/*-end-*/","columns": "auto","view_columns": "","sort": "","name": "修改这里",
                "old_columns": []
            }
        else:
            t_p={"t": "json","pattern": "#"+one_table.attrs['id'],"start": one_table.attrs['id'][0:-len("thetable")] + "_data={'total':'\\d+',\\n'rows':",
                "end": "\\n};\\r\\n\\s+allTableArr.push","columns": "auto","view_columns": "","sort": "","name": "修改这里",
                "old_columns": []
            }            
    else:
        t_p={ "t": "html","name": "修改这里","pattern": "#"+one_table.attrs['id'],"sort": "","columns": "auto","view_columns":"","old_columns":[]}
        #,"start": "4","end": "-3"
    ...
    return t_p,(''.join([x.text for x in one_table.select('tr:nth-child(1) td')])) .replace(u'\xa0', u' ').strip()
...
def __expland_merge_cells(soup,h_pattern):
    ...
    select_result=soup.select(h_pattern)
    if h_pattern.startswith('#'):#easyui的固定行列会导致有多个table，我们将他们的数据按行合并起来
        if len(select_result)>0 and select_result[0].has_attr('data-options'):
            all_heads=[x for x in [__expland_merge_cells(one,'tr') for one in select_result[0].select('thead')] if len(x)>0] #可以排除没有固定列的情况
    ...
    for i_tr, tr in enumerate(select_result):
        lj_col=0
        for td in tr.select("td") + tr.select("th"):
            i_col=int(td.attrs['colspan']) if(td.has_attr('colspan')) else 1
            i_row=int(td.attrs['rowspan']) if(td.has_attr('rowspan')) else 1
            #保证有充足的地方存放数据
                ...
                for y in range(i_col):
                    ...
                    try:
                        while(lj_col+y>=len(idex_t[i_tr+x])):
                             idex_t[i_tr+x]=idex_t[i_tr+x]+[None]
                        while(idex_t[i_tr+x][lj_col+y]!=None):#找到第一个None的地方，填充
                            lj_col=lj_col+1
                            while(len(idex_t[i_tr+x]) < lj_col+i_col ):
                                idex_t[i_tr+x]=idex_t[i_tr+x]+[None]
                        ...
                        print(e)
                    #存放数据
                    idex_t[i_tr+x][lj_col+y]="".join(td.text.split())#去除特殊字符的专业写法 "".join(str.strip(td.text)) #td.text.split()
            lj_col=lj_col+i_col
    return idex_t
...
def __guess_col_names(all_lines,rule_str=None,end_line=None):
    ...
    '''
    if(end_line==None):end_line=len(all_lines)
...
def appendData_and_execLastSql(one_ds,ret,upload_path):
    ...
    if(v['p'].get('vis_sql_conf')!=None and v['p']['vis_sql_conf'].get('expr','').strip()!=''):
...
def load_all_data(config_data,id,appendFunDict=None,args=None,upload_path=None,userid=None):
    ...
    if(config_data.get("vars")!=None):#先计算所有不是依赖excel结果的变量，这样在excel中就也可以使用变量了
        for one_var in config_data["vars"]:
            if(ds_dict.get(one_var["name"])!=None):
...
def files_template_exec(id,config_data,userid,app_save_path,appendFunDict=None,wx_queue=None):
    ...
    def loop_one_txt(one_part,t_ds_dict,idx=0):
        expr_html=BeautifulSoup(one_part['txt'],features="lxml")
        try:
            if expr_html.getText().startswith("http"):
                ...
                txt_tpl=exec_template(None,expr_html.getText(),t_ds_dict) 
            ...
            if expr_html is not None and isinstance(expr_html,BeautifulSoup):
                expr_html.decompose()
                expr_html=None
